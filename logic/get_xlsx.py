...
from settings.gui.windows import helper as hlp
...
class GetXlsx:
    def __init__(self, vmc, hb, rc, st, cd, save_path):
        ...
        self.files_dict = {
            ls.VMC: {
                "path": vmc.split(hlp.PATH_DIVIDER) if vmc else [],
                "check": ls.VMC_CHECK_KEYS
            },
            ls.HB: {
                "path": hb.split(hlp.PATH_DIVIDER) if hb else [],
                "check": ls.HB_CHECK_KEYS
            },
            ls.RC: {
                "path": rc.split(hlp.PATH_DIVIDER) if rc else [],
                "check": ls.RC_CHECK_KEYS
            },
            ls.ST: {
                "path": st.split(hlp.PATH_DIVIDER) if st else [],
                "check": ls.ST_CHECK_KEYS
            },
            ls.CD: {
                "path": cd.split(hlp.PATH_DIVIDER) if cd else [],
