...
class Frame:
    ...
    def create_button_frame(self, buttons_name_and_process):
        """Создает фрейм с кнопками."""
        button_frame = tk.Frame(self.root)
        button_frame.pack(pady=btn.BUTTONS_FRAME_PADY)

        for name, command_name in buttons_name_and_process.items():
            button = tk.Button(
                button_frame,
                text=name,
                command=getattr(self.root, command_name),
                width=btn.BUTTONS_WIDTH
            )
            button.pack(side=tk.LEFT, padx=btn.BUTTONS_PADX)
